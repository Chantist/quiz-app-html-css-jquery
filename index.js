...
const QUIZ_BANK = [{
...

    {
        ...
        options: ['Ayurveda Hair Care', 'Wash Day Routines', 'Henna Gloss Recipes', 'None Of The Above'],
        answer: 'Ayurveda Hair Care'
    },

    {
        question: 'What Is The Benefit Of Doing  ACV Rinses ?',
        options: ['Close Hair Follicles', 'Clarify Hair And Scalp', 'Shiny Hair', 'All The Above'],
...
function generateElement(item) {
    ...
    for (let i = 0; i < item.options.length; i++) {
        appendedVal += `<input type="radio" name="option" value='${item.options[i]}'>${item.options[i]}<br>`;
    }
    ...
    return `
        ...
        <div class= 'question_container js_question'>
          <label class= question_tracker >Question ${quizIndex+1} / ${QUIZ_BANK.length}</label>
          <p class= 'question large_font align_quiz_content'>${item.question}</p>
          
          <div class= 'align_quiz_content answer_selection js_answer_selection'>
          <fieldset>
      <legend class= options_heading>Make A Selection:</legend>
               ${appendedVal}
               </fieldset>
        </div>
        </div>
...
function generateFinalElement() {
    ...
    return `
       ...
       <p class= result_summary>You answered ${scoreCount} out of ${QUIZ_BANK.length} questions correct.<br><br> You scored  <span class= green_emphasise>${calculateTotalScore()}%</span>  on this quiz</p>
